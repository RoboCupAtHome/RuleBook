...
\begin{enumerate}
	...
	\item \textbf{Doing the bed [Optional]:} After the breakfast has been delivered, the robot may proceed to do the owner's bed. Points may be awarded based on TC criteria. If the robot succeed on doing the bed, the jury may add up to 5 minutes to the time for completing the test.
...
\begin{itemize}
	...
	\begin{itemize}
		\item Blanket/duvet grasping
		\item Covering the bed sheet with blanket/duvet
		\item Wrinkles and bearings in the bed sheet and blanket/duvet (the fewer, the better).
		\item Pillow grasping
		\item Placing the pillow in the header of the bed
	\end{itemize}
	Notice that the uses and customs of the host country apply for dimensions and linen for both, beth and pillows. Hence, there is no predefined way for doing bed and robot must adapt to the provided ones. For scoring purposes, the TC may require locals' opinion to check if the bed has been properly done.
\end{itemize}
