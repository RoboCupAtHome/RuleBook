...
The robot must enter the arena, visit each one from a set of waypoints, and leave the arena. 
...
The robot must state when it reached a new waypoint.
...
\begin{enumerate}
	...
	\item \textbf{Waypoint 2 (obstructed path):} After reaching \textit{Waypoint 1}, the robot must navigate and reach (to grasp distance) \textit{Waypoint 2}, which is a placement location. 
	...
	\item \textbf{Waypoint 4 (going back):} Upon reaching \textit{Waypoint 4}, 
	...
	Then, the robot must navigate back to the starting point (\textit{Waypoint 3}).
