...
\begin{itemize}
	...
	\item not trying to exploit the rules (e.g.~not trying to solve the task but trying to score)
	\item not trying to make other robots fail on purpose, and
	\item not modifying standard league robots.
\end{itemize}
...
\begin{enumerate}
	...
	\begin{enumerate}
		\item The team may only instruct the operator when told to do so by a referee.
...
The LOC is responsible for organizing test moderation in the local language.
...
\begin{compactitem}
	\item \textbf{NOT interfere} with the performance
