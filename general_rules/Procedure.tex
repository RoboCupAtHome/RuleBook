...
\begin{enumerate}
	...
	\end{enumerate}
	\ite \textbf{Called by name:} A number of robots is waiting inside the arena, unaccompanied by team members. 
...
\begin{enumerate}
	\item The time limit varies per challenge. The chalenges are intended to be short and should run quickly, e.g. withing a few minutes. 
...
\label{rule:restart}
\begin{enumerate}
	\item Most challenges have no restarts but do feature multiple attempts at a challenge. The scoring system defines an overall score for multiple attempts. 
	...
	\item Some challenges do have restarts, which is indicated per challenge. If this is the case:
	\begin{enumerate}
