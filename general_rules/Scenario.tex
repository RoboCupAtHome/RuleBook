...
The tests take place in the \iterm{RoboCup@Home arena}. Nonetheless, some tests can take place outside the arena, in a previously unknown public place. Rules in this section are related to the \iterm{RoboCup@Home arena} and its interior.
...
\begin{itemize}
	...
	\item Kitchen
\end{itemize}
Depending on the \iterm{Local Organization}, there may be multiple apartments which may be different to each other.
...
Each \iterm{object category} is assigned to a \iterm{predefined location} (e.g. an \textit{fruits} can be found in the \textit{kitchen table}).
...
\begin{itemize}
	...
	\item \textbf{\iterm{Amorphous object}:} A flexible object that may take an infinite number of shapes (e.g. cloth, magnetic puzzle, etc.).
\end{itemize}

\paragraph*{Important note:} It is not allowed to modify any of the objects provided for training.
Teams are not allowed to keep more than 5 of the objects provided for training at a time nor retaining any for more than one hour. The \iaterm{Technical Committee}{TC} will announce where objects were bought so teams can buy their own if they require more time for training.
...
During the competition, objects can be requested based on their \iterm{object category}, its physical attributes, or a combination of both.
...
Room names, predefined locations are announced during setup days (See~\refsec{chap:setup_and_preparation}).
...
All people in the arena have an assigned \iterm{predefined name}.
