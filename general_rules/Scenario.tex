...
\begin{itemize}
	...
	\item a dishwasher in the kitchen (with its rack). 
\end{itemize}
...
\subsubsection{Cutlery, Tableware, and Kitchenware}
\label{rule:tableware_kitchenware}
The TC will provide a set of utensils for kitchen and dinning room related tests. This set includes (but is not limited to) place mats, dishes, bowls, forks, spoons, knives, glasses, mugs, pans, etc. When possible, the TC will make sure that these objects are lightweight, easy to identify, and not fragile (See Figure \ref{fig:scenario_utensils}).

\begin{figure}[H]
	\centering
	\subfloat[Example tableware]{
		\label{fig:scenario_tableware}\includegraphics[width=0.33\textwidth]{images/tableware.jpg}}~
	\subfloat[Kitchenware]{
		\label{fig:scenario_kitchenware}\includegraphics[width=0.33\textwidth]{images/kitchenware.jpg}}
	\\caption{Example utensils}
	\label{fig:scenario_utensils}
\end{figure}

It is allowed to teams to provide their own utensils adapted for the robot. Such utensils must be presented and approved by the TC during robot inspection. Same conditions as with custom containers apply (See \ref{rule:custom_containers}).
