...
\begin{enumerate}
	\item \textbf{Stage~I:} The first days of the competition will be called \iterm{Stage~I}. All qualified teams can participate in \iterm{Stage~I}. Stage~I comprehends a set of \iterm{Ability Tests}, an \iterm{Integration Test}, and an open audience demonstration called \iterm{RoboZoo}. Those \iterm{Proficency Tests} (\iterm{Ability Tests}, and \iterm{Integration Test}) are performed at least 3 times each one.

% MAURICIO: The advance schema changed. Also no demo challenge for 2015
%\item \textbf{Stage~II:} The best \emph{50\% of teams}\footnote{If the total number of teams is less than 20, then the best 10 teams advance to Stage~II.} (after Stage~I) advance to \iterm{Stage~II}. 
...
%  The \iterm{Demo Challenge} is the open---but scoped---demonstration in Stage~II.
	\item \textbf{Stage~II:} The best \emph{50\% of teams with full integrated capabilities}\footnote{If the total number of teams is less than 20, up to 10 teams may advance to Stage~II} (after Stage~I) advance to \iterm{Stage~II}. Here, more complex abilities or combinations of abilities are tested. In order to advance to Stage~II a team must successfully solve 3 out of \iterm{Proficency Tests} in Stage~I. \\
...
\begin{enumerate}
	\item In \iterm{Stage~I}, the \term{maximum number of tests} that a team can participate in is \emph{six (6)}.
	\item In \iterm{Stage~II}, the \term{maximum number of tests} that a team can participate in is \emph{four (4)}.
	\item None of the tests is mandatory, except for the \iterm{Robot Inspection} test (see \refsec{sec:robot_inspection}). % and the \iterm{RoboZoo} test (see \refsec{test:INTHOME}).
...
\begin{enumerate}
	\item \textbf{Stage~I:} The maximum total score (excluding special penalties and bonuses) in \iterm{Stage~I} is \scoring{1000 points}.
	...
	\begin{enumerate}
		\item \textbf{\iterm{Proficency Tests}:} Each proficiency test is run three times. The maximum total score is calculated as the average of the best two attempts for that test.
		\item \textbf{RoboZoo:} The maximum score for \iterm{RoboZoo} is \scoring{50 points}.
	...
	\begin{enumerate}
		...
		\item The maximum score for \iterm{Open Challenge} is \scoring{200 points}.
...
\begin{enumerate}
	\item \textbf{Stage~I:} The \iterm{RoboZoo} is the open demonstrations in \iterm{Stage~I}.
	\begin{enumerate}
