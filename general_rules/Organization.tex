...
\begin{enumerate}
	...
	\item In \iterm{Stage~II}, the \term{maximum number of tests} that a team can participate in is \emph{three (3)}.
	\item None of the tests is mandatory, except for the \iterm{Robot Inspection} test (see \refsec{sec:robot_inspection}).
\end{enumerate}
...
\begin{enumerate}
	...
	\item \textbf{Participation is default:} Teams have to indicate to the \iaterm{Organizing Committee}{OC} in which tests they are \emph{not} going to participate. Wthout such indication, they are automatically added to all test schedules and may receive a penalty when not attending (see \refsec{rule:not_attending}).
