...
\begin{enumerate}
	...
	All qualified teams can participate in \iterm{Stage~I}. Stage~I comprehends of a set of \iterm{Proficiency Tests}.
	Those \iterm{Proficiency Tests} are performed multiple times (See \refsec{rule:score_system}). 
...
% MAURICIO @2017: With the inclusion of SPL only 6 teams per league advance to the second stage (leagues are intended to have 13 teams).
	\item \textbf{Stage~II:} The best \emph{50\% of teams with full integrated capabilities}\footnote{If the total number of teams is less than 12, up to 6 teams may advance to Stage~II} (after Stage~I) advance to \iterm{Stage~II}. Here, more complex abilities or combinations of abilities are tested. 
	
% In order to advance to Stage~II a team must successfully solve 3 out of \iterm{Proficiency Tests} in Stage~I.
% Johannes for this to work we need to explicitly label proficency tests and define what solving means 

	The \iterm{Open Challenge} is an open demonstration in Stage~II.
% MAURICIO @2017: With the inclusion of SPL, we want the finals with no more than 6 teams. At this point, the best two of each league advance to the finals.
	\item \textbf{Final demonstration:} The best \emph{four teams} of each league, the ones with the highest score after Stage~II, advance to the final round. The final round features only a single open demonstration.
\end{enumerate}
...
None of the tests is mandatory, except for the \iterm{Robot Inspection} test (see \refsec{sec:robot_inspection}). However, in order to participate in the finals, a team must have participated in at least one test of Stage~II.
...
\begin{enumerate}
	\item \textbf{Stage~I:} The maximum total score (excluding special penalties and bonuses) in \iterm{Stage~I} is \scoring{885 points} for OPL and DSPL and \scoring{920 points} for SSPL.
	...
	\begin{enumerate}
		\item \textbf{\iterm{Proficiency Tests}:} Each proficiency test is attempted three times unless the time schedule only allows for fewer attempts. The maximum total score is calculated as the average of the best two attempts for that test.
		% \item \textbf{Poster Session:} The Poster Session score is counted in Stage~I and may grant up to \scoring{50 points}.
	\end{enumerate}

	\item \textbf{Stage~II:} The maximum total score (excluding special penalties and bonuses and the \iterm{Open Challenge}) in \iterm{Stage~II} is \scoring{935 points} for OPL and DSPL and \scoring{975 points} for SSPL.
	\begin{enumerate}
		\item The score for \iterm{Open Challenge} depends on the highest achieved score in \iterm{Stage~II} (see \refsec{sec:test_open_challenge:scoring}).
	\end{enumerate}
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\item The formation of the jury is explained in \refsec{final:jury}.
		%\item There are two juries---an \emph{external} consisting of three or more people not from the RoboCup @Home league, and an \emph{internal} formed by the \iaterm{Executive Committee}{EC}. Both juries have different sets of evaluation criteria.
		% Johannes this
