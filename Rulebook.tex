...
\begin{itshape}
Challenges are developed to be repeatable, easy to set up and score and should run quickly.
Each challenge features a real-world relevant task or number of such tasks. 
Each task can be achieved by performing a number of subtasks which grant a small number of points. 
Achieving the complete task however grants larger number of points, regardless of the score on subtasks. 
For some tasks, the subtasks are specified while for other the robot is free to choose how exactly to achieve the overall task. 

One of the challenge is intended as an open demonstration for the audience. 

Each challenge involves a number of skills (e.g. navigation, manipulation etc.) integrated to perform tasks. 
Data is collected for benchmarking and later reference. 

The total score for challenges is the average of the best two performances out of preferably three performances (given the time constraints of a competition).
...
\begin{itshape}
Some tests have optional tasks that grant additional points when performed correctly, clean and fast.
The \iaterm{Technical Committee}{TC} must be informed if a team is planning to perform any of the optional tasks. 
Unless explicitly stated otherwise, no additional time is given while performing optional tasks.
