...
\hline
Collision avoidance (A TC member steps in front of the robot)&\hspace{7cm} & \hspace{7cm} \\
\hline
...
\hline
Usage of continue rule (The robot must stop anywhere between the entry and exit doors and wait for someone to say ``Continue''. 
  Instead of actually saying anything, the team must demonstrate to the TC how to use the continue rule for the robot.) &\hspace{7cm} & \hspace{7cm} \\
\hline
Other safety issues (e.g. cables hanging loose etc.) &\hspace{7cm} & \hspace{7cm} \\
\hline
Custom containers,  if used by the team, need to be shown and approved. \ref{rule:scenario_objects} &\hspace{7cm} & \hspace{7cm} \\
\hline
\end{tabular}\\[1cm]
