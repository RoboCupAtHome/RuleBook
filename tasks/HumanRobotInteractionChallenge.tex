...
The robot is working as a receptionist at a party. The robot has to take two new guests to the living room, offer them drink, offer a free place to sit and introduce them to eachother.\\
    
\textbf{Main goal:}
    The robot welcomes and assists two newcomers at a party, suggesting a drink and offering them a seat, and maintaining appropriate gaze direction during conversation (at person speaking, direction of navigation).
...
If the favorite drink is available the robot must tell the guest the favorite drink is available on the beverage area and where it is. If it is not available, the robot must inform the guest their favorite drink is not available and list all the available drinks in the beverage area.
...
\begin{itemize}
    ...
    \item \textbf{Finding the drink:} The robot shows both guests the beverage area, asks for their favorite drink, and tells them if and where the drink is available on thre table (e.g. left, center right is enough or show in display with bounding box). 
