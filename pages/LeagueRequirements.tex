...
\begin{itemize}
    \item Number of Arenas: 2 (plus 1 Testing Arena)
...
Each arena should be around \qtyrange{90}{120}{\meter\squared}. A minimum of \qty{80}{\meter\squared} is acceptable if robot navigation space is ensured. Typical room sizes are approximately \qtyproduct{4.5 x 4.5}{\metre}.
...
\begin{itemize}
    ...
    \item a trash bin,
    \item a bookcase (or similar shelving) with a minimum shelf spacing of \qty{30}{\centi\metre}; access to at least the lower three shelves must be blocked by two side-by-side doors with U-shaped handles,
    \item a front-loading washing machine,
    \item kitchen equipment including:
    \begin{itemize}
        \item a dishwasher,
        \item a sink,
        \item a pantry (tall cupboard with shelves and U-shaped handles).
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item Ensure one entry door opens inward and one exit door outward.
    \item Avoid loose cables or uneven surfaces.
