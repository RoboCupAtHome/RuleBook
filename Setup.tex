...
\paragraph{Regular tests are not conducted during setup \& preparation.} The competition starts after the Robot Inspection \refsec{sec:robot_inspection}.
...
\begin{table}[h]
  ...
  \begin{center}
    \caption{Schedule (distribution of tests over days may vary)}
    \begin{tabularx}{14.56cm}{S|S|S|S|S|S|S|S}
      \hline
      \multicolumn{2}{|M|}{ \cellcolor[HTML]{FFFFC7}Setup \& \newline Preparation} &
      \multicolumn{2}{M|}{ \cellcolor[HTML]{67FD9A}\iterm{Challenges}} &
...
\begin{enumerate}
	...
	\item \textbf{Groups:} Depending on the number of teams, the \iaterm{Organizing Committee}{OC} may form multiple groups of teams (usually two). The OC will assign teams to groups and announce the assignment to the teams.
...
\begin{enumerate}
	...
	\end{matrix}\right.
	$$
	\item \textbf{Sheet collection:} Evaluation sheets are collected by the OC at a later time (announced beforehand by the OC), allowing teams to continue knowledge exchange during the first days of the competition.
...
\begin{enumerate}
	...
	\begin{itemize}
		...
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		%
		% Standarized continue rule
		%
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		\item usage of (standardized) continue rule (\refsec{rule:asrcontinue}). The robot must stop anywhere between the entry and exit and wait for someone to show a QR-code encoding the text ``Continue'' as per the standardized Continue-rule. 
		  If the team has a custom alternative to ASR for the Continue Rule, this must also be demonstrated so the TC. The TC will decide after deliberation what (reduced) penalty will be given when the alternative is used. 
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		%
		% Non Standarized continue rule
		%
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		% \item usage of continue rule \refsec{rule:asrcontinue}. The robot must stop anywhere between the entry and exit and wait for someone to say ``Continue''. Instead of actually saying anything, the team must demonstrate to the TC how to use the continue rule for the robot. 
